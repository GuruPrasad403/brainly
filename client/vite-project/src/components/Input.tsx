...
function Input(prop : InputTypes):JSX.Element{
    return(
        <div className="flex justify-around items-start w-full h-full gap-5 flex-col my-1 font-semibold">
            ...
            <input type={prop?.type} value={prop?.value} name={prop?.name} autoComplete="off" onChange={prop?.handelChange} placeholder={prop?.placeholder} className="outline-none border-2 px-5 py-2 w-full rounded-2xl"  />
